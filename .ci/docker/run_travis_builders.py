...
def _run_config(tag, clone_dir, commit, level=logging.DEBUG):
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        image = 'dunecommunity/pylrbms-testing_{}:master'.format(tag)
        _cmd(['docker', 'pull', image], logger)
        with NamedTemporaryFile('wt') as envfile:
            envfile.write(env_tpl.format(slug, commit))
            cmd = ['docker', 'run', '--env-file', envfile.name, '-v', '{}:/root/src/pylrbms'.format(os.getcwd()),
                   image, '/root/src/pylrbms/.travis.script.bash']
            try:
                _ = _cmd(cmd, logger)
...
slug = 'dune-community/pylrbms'
commit = subprocess.check_output(['git', 'rev-parse', 'HEAD']).decode().strip()
level = logging.DEBUG if '-v' in sys.argv else logging.INFO
logging.basicConfig()
with TemporaryDirectory() as clone_tmp:
    clone_dir = os.path.join(clone_tmp, 'pylrbms')
